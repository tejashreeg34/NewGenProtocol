...
function App() {
  const { data, updateField, activeTab, setActiveTab, openModal, isAuthenticated, clearSectionData } = useProtocol();
  ...
  const handleGlobalSave = async () => {
    const toastId = toast.loading('Saving progress to database...');
    try {
      // Create a clean payload from context data
      const payload = { ...data };
      
      const response = await axios.post('http://localhost:8000/api/save-protocol', payload);
      
      if (response.data && response.data.protocol_id) {
        // Update the context with the global PGT ID
        updateField('id', response.data.protocol_id);
        toast.success(`Progress saved successfully! ID: ${response.data.protocol_id}`, { id: toastId });
      } else {
        toast.success('Progress saved locally', { id: toastId });
      }
    } catch (error) {
      console.error('Error saving protocol:', error);
      toast.error('Failed to save progress to database. Ensure server is running.', { id: toastId });
    }
  };
