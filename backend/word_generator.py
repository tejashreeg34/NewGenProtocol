...
def create_protocol_table(doc, protocol_data):
    """Create the protocol information table on title page"""
    table_data = [
    ...
    for i, (label, value) in enumerate(table_data):
        cell = table.cell(i, 0)
        cell.text = label
        cell.paragraphs[0].style = 'NormalText'
...
def add_labeled_paragraph(doc, label, value, style='NormalText'):
    """Helper to add a paragraph with a bold label and regular text value."""
...
def create_soa_table(doc, soa_data):
    ...
    table = doc.add_table(rows=1, cols=len(soa_data['table']['headers']) + 1)
    table.style = 'Table Grid'

    hdr_cells = table.rows[0].cells
    hdr_cells[0].text = "Procedures"
    for i, header in enumerate(soa_data['table']['headers']):
        hdr_cells[i+1].text = header

    for procedure, row in soa_data['table']['rows'].items():
...
def generate_complete_word_document(protocol_data):
    ...
    # ========== TITLE PAGE ==========
    title = doc.add_paragraph(protocol_data.get('protocol_title', 'Clinical Trial Protocol'), style='ProtocolTitle')
    ...
    if protocol_data.get('synopsis'):
        create_synopsis_section(doc, protocol_data['synopsis'])
    ...
    if '2' in sections:
        ...
        if sections['2'].get('subsections', {}).get('0'):
            doc.add_paragraph(sections['2']['subsections']['0'], style='NormalText')

        doc.add_heading('2.2 Background', level=2)
        if sections['2'].get('subsections', {}).get('1'):
            doc.add_paragraph(sections['2']['subsections']['1'], style='NormalText')

        doc.add_heading('2.3 Risk/Benefit Assessment', level=2)

        doc.add_heading('2.3.1 Known Potential Risks', level=3)
        if sections['2'].get('subsections', {}).get('2'):
            doc.add_paragraph(sections['2']['subsections']['2'], style='NormalText')

        doc.add_heading('2.3.2 Known Potential Benefits', level=3)
        if sections['2'].get('subsections', {}).get('3'):
            doc.add_paragraph(sections['2']['subsections']['3'], style='NormalText')

        doc.add_heading('2.3.3 Assessment of Potential Risks and Benefits', level=3)
        if sections['2'].get('subsections', {}).get('4'):
            doc.add_paragraph(sections['2']['subsections']['4'], style='NormalText')
    ...
    for section_num in range(4, 12):
        ...
        if section_key in sections:
            ...
            # User override (if any, though usually 'title' key matches template)
            if sections[section_key].get('title'):
                # Check if it differs significantly or just use it
                pass 
                # Ideally we trust the template title format "N TITLE"
                # But sections[key]['title'] might just be "STUDY DESIGN"
                # So let's stick to the constructed one if possible, or use the one from data if it's custom?
                # The user's prompt implies "subsection where already given in the main.py"
                # So we should prefer the TEMPLATE titles.
            ...
            if section_num == 10:
                ...
                elif sections[section_key].get('abbreviations'):
...
def generate_pdf_document(protocol_data):
    ...
    info_items = [
        f"<b>Protocol Number:</b> {protocol_data.get('protocol_number', '')}",
        f"<b>NCT Number:</b> {protocol_data.get('nct_number', '')}",
        f"<b>Principal Investigator:</b> {protocol_data.get('principal_investigator', '')}",
        f"<b>Sponsor:</b> {protocol_data.get('sponsor', '')}",
        f"<b>Funded By:</b> {protocol_data.get('funded_by', '')}",
        f"<b>Version:</b> {protocol_data.get('version_number', '')}",
        f"<b>Date:</b> {protocol_data.get('protocol_date', datetime.now().strftime('%d %B %Y'))}"
    ]
